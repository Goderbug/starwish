/*
  ...
  1. New Tables
    - `users` - User profiles with Google OAuth
    - `wishes` - User wishes stored in cloud
    - `star_chains` - Shareable wish collections
    - `star_chain_wishes` - Many-to-many relationship
    - `blind_box_opens` - Records of opened wishes
    - `user_opened_wishes` - User's collection of received wishes

  2. Security
    - Enable RLS on all tables
    - Add policies for user data isolation
    - Anonymous access for blind box opening

  3. Features
    - Share tracking for creators
    - Opened wish collection for receivers
    - Privacy-preserving anonymous opens
*/
...
CREATE POLICY "Users can manage own star chain wishes"
  ON star_chain_wishes
...
CREATE POLICY "Users can manage wishes by fingerprint"
  ON user_opened_wishes
  FOR ALL
  TO anon, authenticated
  USING (TRUE); -- We'll handle filtering in application logic
